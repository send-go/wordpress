...
- **플러그인 버전 헤더 정정** — 1.3.0 은 `Version:` 헤더와 `SENDGO_VERSION`,
  `readme.txt` 의 `Stable tag` 가 1.2.4 에 멈춘 채 나갔습니다. 워드프레스는 그
  헤더로 업데이트 여부를 판단하므로 갱신이 안내되지 않았습니다.
- 본인인증 관련 설명 정정 — 휴대폰 발신번호는 신분증 사본을 첨부해 API 로
  접수할 수 있고, 카카오 채널 인증번호도 워드프레스 화면에서 끝납니다.
...
```php
// 서버 환경변수에서만 토큰을 읽습니다.
$account = Sendgo_Plugin::instance()->account_client(getenv('SENDGO_AGENT_TOKEN'));
$status = $account->me();
```
...
$client = Sendgo_Plugin::instance()->client();
if ($client !== null) {
    $folders = $client->templateFolders->list(['templateType' => 'notice']);
}
```
